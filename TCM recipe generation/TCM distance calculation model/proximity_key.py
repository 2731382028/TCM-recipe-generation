...
class Network_Distance:
    ...
    def cal_closest(self):
        ...
        # 处理 nodes_from 到 nodes_to 的最短距离
        for node_from in self.nodes_from:
            distances = list(self.length_dict_AB[node_from].values())
            valid_distances = [d for d in distances if d is not None and d != float("inf")]  # 过滤无效值
            if valid_distances:  # 确保列表不为空
                values_outer.append(min(valid_distances))
    
        # 处理 nodes_to 到 nodes_from 的最短距离
        for node_to in self.nodes_to:
            distances = list(self.length_dict_BA[node_to].values())
            valid_distances = [d for d in distances if d is not None and d != float("inf")]  # 过滤无效值
            if valid_distances:  # 确保列表不为空
                values_outer.append(min(valid_distances))
    
        # 如果没有有效距离，返回 None 或特定值
        if not values_outer:
    ...
    def network_distance(self, distance_method):
        ...
        if distance_method == 'separation':
            ...
            raise ValueError(f"无效的距离计算方法: {distance_method}")
        if distance is None:
            return 0  # 或 raise ValueError("无法计算距离")
        return distance
